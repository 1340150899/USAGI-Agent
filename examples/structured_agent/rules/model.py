...
from usagi_agent.pipelines.rules import ModelRuleAdapterConfig, StatePatch
...
class ResearchWriterModelConfig(ModelRuleAdapterConfig):
    ...
    async def model(self, state: AgentRunState, runtime, context: RunContext) -> StatePatch:
        ...
        context_ref = ArtifactRef(
            artifact_id=state.get("context_pack_ref", ""), content_type="application/json"
        )
        ...
        ref = await put_model(
            ...
            operation_id=f"model:{context.run_id}:{state.get('iteration', 0)}",
        )
        return {"model_response_ref": ref}
