...
export async function sendMessageWeixin(params: {
  ...
  try {
    ...
    const serverId = response.message_id ?? response.msg_id;
    return {clientId, messageId: serverId === undefined ? undefined : String(serverId), response};
  } catch (err) {
...
export async function sendMessageItemWeixin(params: {
  ...
  try {
    ...
    return {
      clientId,
      messageId: serverId === undefined ? undefined : String(serverId),
      response,
    };
  } catch (err) {
    logger.error(
...
async function sendMediaItems(params: {
  ...
  opts: WeixinMessageSendOptions;
  label: string;
}): Promise<{ clientIds: string[]; messages: SentMessageResult[] }> {
  ...
  const messages: SentMessageResult[] = [];
  for (const item of items) {
    ...
    try {
      ...
      logger.error(`${label}: failed to=${to} clientId=${clientId} err=${String(err)}`);
      throw err;
    }
  }

  logger.info(`${label}: success to=${to} clientIds=${clientIds.join(",")}`);
  return { clientIds, messages };
}
