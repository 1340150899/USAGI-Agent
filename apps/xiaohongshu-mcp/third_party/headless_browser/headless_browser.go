...
func newDefaultConfig() *Config {
	return &Config{
		Headless:      true,
		// Empty means "use the browser's native user agent", which keeps UA,
		// client hints and other fingerprints self-consistent.
		UserAgent:     "",
		Cookies:       "",
...
func New(options ...Option) *Browser {
	...
	l := launcher.New().
		...
		// Windows Defender flags the leakless helper exe as a virus (false
		// positive); launch the browser directly without the wrapper.
		Leakless(false)

	// Only override the user agent when explicitly requested. A hardcoded UA
	// (e.g. Mac Chrome on a Windows host) contradicts client hints, fonts and
	// WebGL fingerprints, which is itself a detectable automation signal.
	if cfg.UserAgent != "" {
	...
	browser := rod.New().
		ControlURL(url).
		// rod emulates a "Laptop with MDPI screen" device on every page by
		// default, which overrides the user agent with a hardcoded Mac
		// Chrome/114 string (and pins the viewport to 1280). Disable it so
		// pages report the browser's real, self-consistent environment.
		NoDefaultDevice().
		Trace(cfg.Trace).
		MustConnect()
