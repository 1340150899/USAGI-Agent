...
> 应用接入的当前实现见 [微信应用接入与工具审批](application-integration.md)：HTTP Server 为应用入口，独立 Node 微信 Adapter 直连 USAGI，审批由 ToolManager 与 LangGraph 执行。下文保留原业务设计背景；与当前接入方式冲突处以该实施文档为准。
...
13. `run_id`、thread/checkpoint ID、ArtifactRef 和 resume token 均不是凭据。对外只通过 `Server.create_session/continue_session/get_run/resume/issue_resume_token/cancel` 从服务端认证上下文执行 `run.start/read/resume/cancel` 及 interrupt scope 检查；GET 不返回明文 token，审核通知只放认证 UI 链接。当前版本不公开事件流和 token 补发接口。
