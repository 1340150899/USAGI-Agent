...
| `apps/xiaohongshu-mcp` | 仓库内置 Go MCP 服务；由 HTTP Server 通过 Streamable HTTP 注册 |
| `usagi-agent/.../tools/approval.py` | 通用工具审批，无微信或 QQ 逻辑 |

## 两类输入

Windows 普通消息作为素材。HTTP Server 将素材保存在三态候选池中：`unconsumed`（未消费）、`selected`（已选中）和 `consumed`（已消费）。同一会话收到新素材后连续静默 10 秒，且当前未消费窗口至少包含一张图片时，服务原子选中整个窗口，并用独立会话直接调用通用 `research_writer` 场景，不创建素材任务队列；后续窗口从剩余未消费素材开始。第一轮 Agent 判断素材是否足以形成笔记：不足则窗口静默退回候选池，不再向用户广播说明；足够则生成完整帖子、直接把结果发给用户并询问是否发布，此时不调用工具。用户引用草稿回复时，消息进入同一个 Session，由 Agent 判断是否发起 `xhs_publish_content`；工具调用随后由 Runtime 发起第二次审批，审批通过后才执行。发布成功后消费窗口；用户拒绝、工具审批拒绝或确定未发布时退回，但没有新消息时不会重复判断，必须等新消息移动窗口右边界并再次静默 10 秒。发布结果为 unknown 时保持选中等待人工核验。

自动链路的 Agent 回复以“您的小红书运营助手：”开头用于与普通消息区分；前缀只作为聊天标识，不进入发布工具参数。会话图片按接收顺序累计编号，草稿与修订稿在 answer 末尾以“配图：第1张、第3张”标注选用图片（不配图写“配图：无”），HTTP Server 解析标注后把图片随投递 payload 的 `media_ids` 发给 Adapter，由 Adapter 下载媒体并作为图片消息发出；图片消息的 message_id 与文本一样进入引用映射，用户引用某张图片回复也能找回会话。首发草稿未带标注时回退发送全部窗口图片；会话内后续回复仅在明确标注时带图。图片回传依赖 Adapter 凭据能读取素材图：weixin binding 与 wxauto binding 必须同 principal，否则媒体下载 404、投递 partial。用户在微信里自发图片加任务文字的交互会话没有走素材 prompt，不输出配图标注，也不在此图片回传范围内；上下文压缩后模型沿用编号引用历史图片属预期行为。
